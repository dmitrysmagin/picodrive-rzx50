...
void menu_romload_prepare(const char *rom_name)
{
	...
	if (rom_data) sdl_pd_clone_buffer2();
...
static void draw_dirlist(char *curdir, struct dirent **namelist, int n, int sel)
{
	...
	if (rom_data == NULL) {
		menu_darken_bg(sdl_screen, 320*240, 0);
	}

	menu_darken_bg((char *)sdl_screen + 320*120*2, 320*8, 0);
...
static void menu_loop_root(void)
{
	...
	for (;;)
	{
		...
		if(inp &(GP2X_SELECT|GP2X_X)){
			if (rom_data) {
				while (sdl_joystick_read(1) & (GP2X_SELECT|GP2X_X)) usleep(50*1000); // wait until select is released
		...
		if(inp & GP2X_B)  {
			switch (me_index2id(main_entries, MAIN_ENTRY_COUNT, menu_sel))
			{
				case MA_MAIN_RESUME_GAME:
					if (rom_data) {
						while (sdl_joystick_read(1) & GP2X_B) usleep(50*1000);
						engineState = PGS_Running;
						return;
					}
					break;
